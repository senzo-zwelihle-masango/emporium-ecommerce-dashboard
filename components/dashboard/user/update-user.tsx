...
import { editUserSchema, EditUserSchemaType } from '@/schemas/dashboard/user'
import { Role } from '@/lib/generated/prisma'
import { updateUserAction } from '@/server/actions/dashboard/user'
...
const UpdateUserForm = ({ user }: UpdateUserFormProps) => {
  ...
  function onSubmit(values: EditUserSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(updateUserAction(values, user.id))
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Phone Number (optional)" {...field} value={field.value || ''} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
